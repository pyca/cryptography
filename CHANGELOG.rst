...
* **BACKWARDS INCOMPATIBLE:** Invalid ASN.1 found during :doc:`/x509` parsing
  will raise an error on initial parse rather than when the invalid field is
  accessed.
* **BACKWARDS INCOMPATIBLE:** Values passed to the :doc:`/x509` PEM parsers
  must be a single PEM payload and will error on extraneous data.
* Rust is now required for building ``cryptography``, the
  ``CRYPTOGRAPHY_DONT_BUILD_RUST`` environment variable is no longer
  respected.
* Parsers for :doc:`/x509` no longer use OpenSSL and have been rewritten in
  Rust. This should be backwards compatible (modulo the items listed above)
  and improve both security and performance.

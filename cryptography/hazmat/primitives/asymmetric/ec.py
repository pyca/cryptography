...
class ECDSA(object):
    ...
    def signer(self, private_key):
        return private_key.backend.create_ecdsa_signature_ctx(
            private_key, self.algorithm)

    def verifier(self, signature, public_key):
        return public_key.backend.create_ecdsa_verification_ctx(
            public_key, signature, self.algorithm)


def generate_elliptic_curve_private_key(curve, backend):

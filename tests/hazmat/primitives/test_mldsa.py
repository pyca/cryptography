...
def test_mldsa_unsupported(backend):
    ...
    with raises_unsupported_algorithm(
        ...
        MLDSAMuHasher(b"invalid")  # type: ignore[arg-type]
...
class TestMLDSA:
    ...
    def test_mu_hasher_context_too_long(self, variant, backend):
        ...
        MLDSAMuHasher(pub, b"a" * 255)
        with pytest.raises(ValueError):
            MLDSAMuHasher(pub, b"a" * 256)
    ...
    def test_mu_hasher_invalid_key_type(self, backend):
        ...
        with pytest.raises(TypeError):
            MLDSAMuHasher(b"not a key")  # type: ignore[arg-type]

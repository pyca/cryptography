...
class TestECDHVectors(object):
    ...
    def test_unsupported_ecdh_arguments(self, backend):
        ...
        curve = ec.SECP521R1
        _skip_curve_unsupported(backend, curve)
        pri_key = ec.generate_private_key(curve, backend)
        ecdh = ECDH(pri_key, backend)
        with pytest.raises(TypeError):
            ecdh.compute_key(None, NullKDF())
        with pytest.raises(TypeError):
            ecdh.compute_key(pri_key.public_key(), None)
    ...
    def key_exchange(self, backend, vector):
        ...
        ecdh = ECDH(prikey, backend)
        z = ecdh.compute_key(peerpubkey, NullKDF())

...
from cryptography.hazmat.decrepit.ciphers.algorithms import RC2, TripleDES
...
class TestDERSerialization:
    ...
    @pytest.mark.supported(
        ...
        and backend.cipher_supported(
            TripleDES(b"\x00" * 8), modes.CBC(b"\x00" * 8)
        ),
        skip_message="Does not support DES MD5",
    )
    def test_load_pkcs8_pbe_with_md5_and_des_cbc(self):
        # Test for OID 1.2.840.113549.1.5.3 (pbeWithMD5AndDES-CBC)
        key = load_vectors_from_file(

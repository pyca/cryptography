...
* ``pkcs7/ca/ca.pem`` - A self-signed RSA certificate with ``basicConstraints``
  set to true. Its private key is ``pkcs7/ca/ca_key.pem``. This key is
  encoded is used to try decoding the PKCS7 custom vectors.
...
* ``pkcs7/enveloped-aes-256-cbc.pem`` - A PEM encoded PKCS7 file with 
  enveloped data, encrypted using AES-256-CBC.

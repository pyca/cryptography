...
class _EVPCIPHER:
    """
    Access to AEADs through EVP_CIPHER APIs.
    """
    ...
    @staticmethod
    def evp_cipher(cipher_name: bytes, backend: Backend):
        if cipher_name.endswith(b"-siv"):
            ...
            evp_cipher = backend._ffi.gc(
                evp_cipher, backend._lib.EVP_CIPHER_free
            )
        else:
    ...
    def aead_setup(
        ...
        res = backend._lib.EVP_CipherInit_ex(
            ...
            backend._ffi.NULL,
            backend._ffi.NULL,
            int(operation == _EVPCIPHER._ENCRYPT),
        )
        backend.openssl_assert(res != 0)
        ...
        backend.openssl_assert(res != 0)
        if operation == _EVPCIPHER._DECRYPT:
            assert tag is not None
            _EVPCIPHER.set_tag(backend, ctx, tag)
    ...
    @staticmethod
    def set_tag(backend, ctx, tag: bytes) -> None:
    ...
    @staticmethod
    def set_length(backend: Backend, ctx, data_len: int) -> None:
    ...
    @staticmethod
    def process_aad(backend: Backend, ctx, associated_data: bytes) -> None:
    ...
    @staticmethod
    def process_data(backend: Backend, ctx, data: bytes) -> bytes:
        outlen = backend._ffi.new("int *")
        buf = backend._ffi.new("unsigned char[]", len(data))
        data_ptr = backend._ffi.from_buffer(data)
        res = backend._lib.EVP_CipherUpdate(
            ctx, buf, outlen, data_ptr, len(data)
        )
        if res == 0:
    ...
    def encrypt(
        ...
        if ctx is None:
            ...
            _EVPCIPHER.set_nonce_operation(
                backend, ctx, nonce, _EVPCIPHER._ENCRYPT
            )
        ...
        if isinstance(cipher, AESCCM):
            _EVPCIPHER.set_length(backend, ctx, len(data))

        for ad in associated_data:
            _EVPCIPHER.process_aad(backend, ctx, ad)
        processed_data = _EVPCIPHER.process_data(backend, ctx, data)
        outlen = backend._ffi.new("int *")
    ...
    def decrypt(
        ...
        if ctx is None:
            ...
            _EVPCIPHER.set_nonce_operation(
                backend, ctx, nonce, _EVPCIPHER._DECRYPT
            )
            _EVPCIPHER.set_tag(backend, ctx, tag)

...
class CounterKDF(object):
    def __init__(self, algorithm, length, label, context, backend):
        ...
        if not isinstance(label, bytes) \
            ...
            raise TypeError('Label, Context and IV must be of type bytes')
    ...
    def derive(self, key_material):
        ...
        output = [b'']
        # upside-down floor division (equivalent to math.ceiling)
        rounds = -(-self._length // self._algorithm.digest_size)

        # 8 is the size of an unsigned int used by struct.pack('>I', n)
        if rounds > pow(2, 8) - 1:

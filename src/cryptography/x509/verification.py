...
class CertificateVerificationContext(object):
    def __init__(self, signing_certificate):
        if not isinstance(signing_certificate, Certificate):
            raise InvalidCertificate(
                "The signing certificate must be a Certificate."
            )
        _can_sign_certificates(signing_certificate)

        self._signing_cert = signing_certificate

    def update(self, certificate):
        """
        Processes the provided certificate and returns nothing.
        """
    ...
    def verify(self):
        """
        Raises an exception if the signature of the certificate provided to
        update was not generated by the signing certificate.
        """

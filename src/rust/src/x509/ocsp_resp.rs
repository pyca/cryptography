...
impl OCSPResponse {
    ...
    fn produced_at<'p>(
        ...
        let message = std::ffi::Cstr::from_bytes_with_nul("Properties that return a naïve datetime object have been deprecated. Please switch to produced_at_utc.").unwrap();
    ...
    fn revocation_time<'p>(
        ...
        let warning_cls = types::DEPRECATED_IN_43.get(py)?;
        let message = std::ffi::Cstr::from_bytes_with_nul("Properties that return a naïve datetime object have been deprecated. Please switch to revocation_time_utc.").unwrap();
        pyo3::PyErr::warn(py, &warning_cls, message.as_c_str(), 1)?;
        let resp = self.requires_successful_response()?;
        let single_resp = single_response(resp)?;
    ...
    fn this_update<'p>(
        ...
        let warning_cls = types::DEPRECATED_IN_43.get(py)?;
        let message = std::ffi::Cstr::from_bytes_with_nul("Properties that return a naïve datetime object have been deprecated. Please switch to this_update_utc.").unwrap();
        pyo3::PyErr::warn(py, &warning_cls, message.as_c_str(), 1)?;
        let resp = self.requires_successful_response()?;
        let single_resp = single_response(resp)?;
